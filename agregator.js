...
function agregator_register() {
  let pahoConfig = {
    ...
    agr_clientId: "11jolek11-agregator".concat(
      (Math.random() + 1).toString(36).substring(7)
    ), //Should be unique for every of your client connections.
    keepAliveInterval: 0,
    reconnect: true,
  };

  agr_client = new Paho.MQTT.Client(
    pahoConfig.hostname,
    Number(pahoConfig.port),
    pahoConfig.agr_clientId
  );
  ...
  function agr_onMessageArrived(message) {
    ...
    if (
      JSON.stringify(agregator_global_register.slice(-1)[0]) !==
      JSON.stringify(obj)
    ) {
      const dev = document.getElementById("agregator_registerlist");
      agregator_global_register.push(obj);
      const payload = `<div style="background-color:yellow;",  class="agregator", id=${obj.uuid}>\
...
function change_config_agregator(button) {
  ...
  for (let i = 0; i < agregator_global_register.length; i++) {
    ...
    if (element.uuid == button.value) {
      ...
      var targetInput = document
        .getElementById(`edit-${element.uuid}`)
        .getElementsByClassName("agregator_input");
