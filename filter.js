...
function filter_register() {
  let pahoConfig = {
    ...
    agr_clientId: "11jolek11-filter".concat(
  ...
  const agr_client = new Paho.MQTT.Client(
    pahoConfig.hostname,
  ...
  function agr_onMessageArrived(message) {
    ...
    if (
      JSON.stringify(filter_global_register.slice(-1)[0]) !==
      JSON.stringify(obj)
    ) {
      const dev = document.getElementById("filter_registerlist");
      filter_global_register.push(obj);
      const payload = `<div style="background-color:green;",  class="filter", id=${obj.uuid}>\
  ...
  for (let i = 0; i < filter_global_register.length; i++) {
    ...
    if (element.uuid == button.value) {
      ...
      var targetInput = document
        ...
        .getElementsByClassName("filter_input");
