...
function register() {
  let pahoConfig = {
    ...
    clientId: "11jolek11-panel".concat(
  ...
  function onMessageArrived(message) {
    ...
    if (JSON.stringify(global_register.slice(-1)[0]) !== JSON.stringify(obj)) {
      ...
      const payload = `<div class="item", id=${obj.name}>\
...
function stop_generator(button) {
  for (let i = 0; i < global_register.length; i++) {
    ...
    if (element.name == button.value) {
      fetch(
        `http://${element.ip}:${element.port}/${button.value}/stop`,
        requestOptions
      )
...
function status_generator(button) {
  ...
  for (let i = 0; i < global_register.length; i++) {
    ...
    if (element.name == button.value) {
      fetch(
        `http://${element.ip}:${element.port}/${button.value}/status`,
        requestOptions
      )
  ...
  for (let i = 0; i < global_register.length; i++) {
    ...
    if (element.name == button.value) {
      ...
      var targetInput = document
        .getElementById(`edit-${element.name}`)
